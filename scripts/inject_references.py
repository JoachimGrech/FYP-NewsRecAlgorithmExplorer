...
NEW_OUTPUT_PATH = r"C:\Users\joach\Downloads\FYP_Dissertation_Final_v2.docx"
...
references = [
"[1] European Commission, “Media & News Survey 2023,” europa.eu, 2023. Available: https://europa.eu/eurobarometer/surveys/detail/3153. [Accessed: Sep. 26, 2025]",
...
for p in doc.paragraphs:
    if "Bibliography" in p.text and p.style.name.startswith("Headings"):
        bib_anchor = p._element
        break

if bib_anchor is not None:
    ref_heading = make_para("Headings", "References")
    bib_anchor.addprevious(ref_heading)
    
    for ref in references:
        ref_para = make_para("Dissertation Body", ref)
        bib_anchor.addprevious(ref_para)

    blank_para = make_para("Dissertation Body", "")
    bib_anchor.addprevious(blank_para)
    
    doc.save(NEW_OUTPUT_PATH)
    print(f"Injected 29 references into {NEW_OUTPUT_PATH}")
else:
    print("Could not find Bibliography heading to inject references.")
