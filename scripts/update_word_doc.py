...
for i, p in enumerate(doc.paragraphs):
    if p.style.name == "Heading 1" and "Introduction" in p.text:
        intro_idx = i
    elif p.style.name == "Heading 1" and "Background" in p.text:
        bg_idx = i
    elif p.style.name == "Heading 1" and "Methodology" in p.text:
        meth_idx = i
        break

# Delete everything from Introduction up to Methodology
to_remove = [doc.paragraphs[i]._element for i in range(intro_idx, meth_idx)]
...
# We will inject Introduction, Background, and Literature Review right before Methodology
sections = [
    (H1_ID, "Introduction"),
    (BODY_FIRST_ID, "Recommendation algorithms play a fundamental role in shaping how users discover and consume digital content, particularly in online news. As evidenced by recent Eurobarometer surveys, a significant proportion of Maltese citizens now rely on online platforms and social media as their primary sources of information [1], [2]. This shift highlights the extent to which automated recommendation systems act as primary gatekeepers of information in contemporary society."),
    (BODY_ID, "Despite their growing societal influence, only a limited proportion of the population possesses a strong understanding of how these algorithms operate. While many users are aware that some level of personalisation exists, far fewer understand the mechanisms behind content selection or the long-term implications of algorithmic curation [3]. This knowledge gap is problematic, as recommender systems are closely linked to issues such as filter bubbles, echo chambers, algorithmic bias, manipulation, and unequal representation of viewpoints. In response to these challenges, increased attention is being given to transparency, explainability, and user control within algorithmic systems."),
    (BODY_ID, "Making recommendation mechanisms more understandable to non-experts is essential for fostering critical engagement with digital media. Interactive tools that allow users to explore and experiment with algorithm behaviour offer a promising approach to addressing this issue by exposing the relationship between algorithmic parameters and content visibility. This project focuses on the development of an interactive simulation environment for news recommendation algorithms. By enabling hands-on experimentation with different recommendation strategies, the system aims to provide users with practical insight into how personalised news delivery operates on real-world platforms, as well as the potential social and ethical implications that arise from algorithm-driven information exposure."),
    
    (H2_ID, "Aim and Objectives"),
    (BODY_FIRST_ID, "The main aim of this project is to design and develop an interactive dashboard that allows users to explore, manipulate, and understand the behaviour of news recommendation algorithms in a controlled and transparent environment. To achieve this aim, the following objectives have been defined as:"),
    (BODY_ID, "•	To design and implement simulations of core news recommendation algorithms, including content-based and collaborative filtering approaches."),
    (BODY_ID, "•	To enable real-time manipulation of key algorithmic parameters by users."),
    (BODY_ID, "•	To visualise how changes in algorithmic configuration influence the distribution and diversity of recommended news content."),
    (BODY_ID, "•	To demonstrate the effects of personalisation, bias, and filter bubbles through interactive experimentation."),
    (BODY_ID, "•	To develop and evaluate visual explainability interfaces (such as multi-dimensional radar charts) that transparently communicate the rationale behind specific news recommendations based on aggregated user reading history."),
    (BODY_ID, "•	To improve user awareness of the societal and ethical implications of algorithm-driven news delivery."),

    (H2_ID, "Document Structure"),
    (BODY_FIRST_ID, "This section will describe the overall structure of the dissertation and provide a brief overview of the contents of each chapter."),
...
# Now fix Abstract text
# Abstract is located before Introduction
abstract_idx = None
for i, p in enumerate(doc.paragraphs):
    if "Abstract" in p.text and p.style.name.startswith("Heading"):
        abstract_idx = i
        break

if abstract_idx is not None:
    # Remove the 3 paragraphs following Abstract
    # In the original, there were some paragraphs. Let's find where Acknowledgements starts.
    ack_idx = None
    for i in range(abstract_idx + 1, len(doc.paragraphs)):
        if "Acknowledgements" in doc.paragraphs[i].text:
            ack_idx = i
            break
            
    if ack_idx is not None:
        to_remove = [doc.paragraphs[i]._element for i in range(abstract_idx + 1, ack_idx)]
        for elem in to_remove:
            elem.getparent().remove(elem)
        
        ack_anchor = doc.paragraphs[ack_idx]._element
        
        abs_sections = [
            (BODY_FIRST_ID, "With the rapid growth of digital news consumption, recommendation algorithms have become a central mechanism through which users access information. According to a 2023 Eurobarometer survey, 49% of Maltese citizens obtain their news from online news platforms, while 70% rely on social media platforms [1]. A subsequent 2025 Eurobarometer survey reported that 74% of Maltese citizens receive information on current social and political affairs from social media, while 38% use printed newspapers or their online versions and 43% rely on search engines, compared with 48% who obtain their information from television [2]. These statistics highlight the dominant role of algorithm-driven platforms in modern news consumption."),
            (BODY_ID, "Despite this influence, public understanding of recommendation algorithms remains limited. Only 9% of EU citizens claim to know a lot about recommendation systems, while 44% report knowing little about them. Furthermore, only 41% of EU citizens were aware in 2019 that algorithms are used to personalise news and information [3]. This lack of awareness raises serious concerns related to filter bubbles, confirmation bias, manipulation, and data exploitation."),
            (BODY_ID, "This project proposes the development of an interactive news recommendation algorithm explorer in the form of a dashboard. Unlike traditional \"black-box\" systems, this dashboard allows users to manipulate key algorithmic parameters and observe, in real time, how these changes affect the content they are exposed to. Crucially, the system introduces visual explainability mechanisms, such as multi-dimensional radar charts mapping user profiles against article topics, to transparently communicate why specific recommendations are made. By simulating different recommendation strategies and making their internal logic visible and interpretable, the project seeks to foster a deeper public understanding of how algorithmic systems shape information access.")
        ]
        
        for style_id, text in abs_sections:
            new_elem = make_para(style_id, text)
            ack_anchor.addprevious(new_elem)
